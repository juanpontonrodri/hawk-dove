...
class QLearningNeuron {
    ...
    public QLearningNeuron(int x, int y) {
        posX = x;
        posY = y;
        qTable = new HashMap<>();
        random = new Random();
...
class SelfOrganizingMap {
    ...
    private double[][][] grid;
    public QLearningNeuron[][] neuronGrid;

    public SelfOrganizingMap(int gridSide, int inputDim) {
        gridSize = gridSide;
        inputSize = inputDim;
        radius = gridSize / 10;
        grid = new double[gridSize][gridSize][inputSize];
        timesBMU = new int[gridSize][gridSize];
        neuronGrid = new QLearningNeuron[gridSize][gridSize];
        resetValues();
    }
    ...
    public void resetValues() {
        ...
        for (int i = 0; i < gridSize; i++) {
            for (int j = 0; j < gridSize; j++) {
                neuronGrid[i][j] = new QLearningNeuron(i, j);
...
public class NN_Agent extends Agent {
    ...
    protected void setup() {
        som = new SelfOrganizingMap(GRID_DIMENSION, INPUT_DIMENSION);
